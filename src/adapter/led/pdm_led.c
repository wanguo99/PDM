...
static int pdm_led_device_probe(struct pdm_device *pdmdev)
{
    ...
    status = pdm_client_add(led_adapter, pdmdev);
    if (status) {
...
err_client_del:
    pdm_client_delete(led_adapter, pdmdev);
...
static void pdm_led_device_remove(struct pdm_device *pdmdev)
{
    ...
    status = pdm_client_delete(led_adapter, pdmdev);
    if (status) {
...
int pdm_led_driver_init(void)
{
    ...
    led_adapter = pdm_adapter_alloc(sizeof(struct pdm_led_priv));
    if (!led_adapter) {
        OSA_ERROR("Failed to allocate pdm_adapter\n");
        return -ENOMEM;
    }

    status = pdm_adapter_register(led_adapter, PDM_ADAPTER_LED_NAME);
    if (status) {
